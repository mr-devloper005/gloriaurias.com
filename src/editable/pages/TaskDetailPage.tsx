...
// ----- Bookmark: source dossier with an anchored metadata rail -----
  ...
  return (
    <>
      <section className="border-b border-[var(--tk-line)] bg-[var(--tk-text)] text-white">
        <div className="mx-auto grid max-w-[var(--editable-container)] gap-10 px-6 py-14 sm:py-20 lg:grid-cols-[300px_minmax(0,1fr)] lg:px-8 lg:py-24">
          <aside className="rounded-[var(--tk-radius)] border border-white/15 bg-white/5 p-6 lg:sticky lg:top-24 lg:self-start">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--tk-accent)] text-[var(--tk-text)]">
              <Bookmark className="h-5 w-5" />
            </div>
            <p className="mt-8 text-xs font-semibold uppercase tracking-[0.24em] text-[var(--tk-accent)]">Source dossier</p>
            ...
            {website ? (
              <Link href={website} target="_blank" rel="noreferrer" className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--tk-accent)] px-5 py-3 text-sm font-semibold text-[var(--tk-text)] transition hover:-translate-y-0.5 hover:bg-white">
                Visit resource <ExternalLink className="h-4 w-4" />
              </Link>
            ) : null}
          </aside>

          <article className="min-w-0 self-end">
            <Kicker task="sbm">Curated resource</Kicker>
            <h1 className="editable-display mt-6 max-w-5xl text-balance text-5xl font-semibold leading-[0.96] tracking-[-0.03em] sm:text-6xl lg:text-7xl">{post.title}</h1>
            {leadText(post) ? <p className="mt-7 max-w-3xl text-lg leading-8 text-white/70">{leadText(post)}</p> : null}
            {tags.length ? (
              <div className="mt-8 flex flex-wrap gap-2">
                {tags.map((tag) => <span key={tag} className="rounded-full border border-white/15 px-3 py-1.5 text-xs font-semibold text-white/65">{tag}</span>)}
              </div>
            ) : null}
          </article>
        </div>
      ...
      <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-6 py-12 sm:py-16 lg:grid-cols-[minmax(0,1fr)_340px] lg:px-8">
        <article className="min-w-0 rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6 sm:p-9">
          <div className="flex flex-col gap-4 border-b border-[var(--tk-line)] pb-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[var(--tk-accent)]">Resource notes</p>
              <h2 className="editable-display mt-2 text-3xl font-semibold tracking-[-0.02em]">Context and details</h2>
            </div>
            <span className="inline-flex w-fit items-center gap-2 rounded-full bg-[var(--tk-accent-soft)] px-4 py-2 text-sm font-semibold">
              <Globe2 className="h-4 w-4" /> {domain}
            </span>
          </div>
          <BodyContent post={post} />
        </article>
        ...
        <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-raised)] p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[var(--tk-muted)]">Quick read</p>
           
            <div className="mt-5 grid gap-3 text-sm text-[var(--tk-muted)]">
              <p className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[var(--tk-accent)]" /> Reviewed for usefulness</p>
              <p className="inline-flex items-center gap-2"><Tag className="h-4 w-4 text-[var(--tk-accent)]" /> Filed in {collection}</p>
            </div>
          </div>
